...
export const Transaction: React.FC<{}> = () => {
	...
	useEffect(() => {
		filter ? setRecData(data.signals?.filter((elem) => elem.status === filter)) : setRecData(data.signals);
	}, [filter]);
						...
						<li key={index + 1} className='recomendation__item'>
							<div className='recomendation__item'>
								...
								<div className='recomendation__wrapper'>
									{loaded ? (
										<div className='skeleton-box' style={{width: '80px', height: '29px'}}></div>
									) : (
										<>
											<img src={`./images/${elem.currency.toLowerCase()}.png`} alt={`Coin name ${elem.currency}`} />
											<div>
												<h6>{elem.currency} </h6>
												<p>{elem.shortname}</p>
											</div>
								...
								{loaded ? (
									...
									<p className='recomendation__orderStatus' style={{color: checkStatus(elem.status)}}>
								...
								<p className='recomendation__costs recomendation__costs--enter'>
									{elem.entry_price_range.map((elem, index) => (
										<>
											{loaded ? (
												<div className='skeleton-box' key={index} style={{marginBottom: '5px'}}>
													${elem}
												</div>
											) : (
