			...
			<div className='payment__form'>
				<div className='payment__inner'>
					<div className='payment__wrapper payment__wrapper--form'>
						{['sum', 'wallet'].map((elem) => (
							<div>
								<h4 className='payment__label'>{elem === 'sum' ? 'Сумма' : 'Кошелёк'}</h4>
								<input
									...
									type='text'
								/>
								<button className='payment__copy' onClick={() => handleCopy(elem === 'sum' ? sumRef : walletRef)}>
				...
				<div className='payment__txid'>
					...
					<form className='payment__operation' action='' onSubmit={handleSubmit(onSubmit)}>
						<input
							style={{
						...
						/>
						<p className={errors.operationId?.message ? 'payment__error active' : 'payment__error'}>
